...
import static com.tencent.liteav.demo.play.controller.TCVodControllerBase.isWifi;
...
import com.tencent.liteav.demo.play.controller.ConsumptionFlowOBAlertDialog;
...
//    private FloatVideoPlayerView floatVideoView;
//    private TCVodControllerFloat mVodControllerFloat;
    ...
    private int mCurrentPlayType = SuperPlayerConst.PLAYTYPE_VOD;
    ...
    private int mVodPlayerMute;
    private boolean isList;
    private int uiOptions = View.SYSTEM_UI_FLAG_HIDE_NAVIGATION
            | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN;
    private int transparencyUiOptions = View.SYSTEM_UI_FLAG_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
    private boolean isTransparencyUiOptions = false;
    private ServiceConnection serviceConnection;
    private Activity activity;
    private Bundle bundle;
    private MediaService.OnClickFloatCloseBtnListener onClickFloatCloseBtnListener;
    ...
    public void playVodVideo(SuperPlayerModel superPlayerModel, boolean hasBack, boolean isTransparencyUiOptions, Bundle bundle) {
        this.bundle = bundle;
        playVodVideo(superPlayerModel, hasBack, isTransparencyUiOptions);
    }

    public void playVodVideo(SuperPlayerModel superPlayerModel, boolean hasBack, boolean isTransparencyUiOptions) {
        if (activity != null) {
            if (serviceConnection != null) {
                mediaService.stopPlay();
                playWithModel(superPlayerModel, hasBack, isTransparencyUiOptions);
            } else {
                serviceConnection = new ServiceConnection() {
                    @Override
                    public void onServiceConnected(final ComponentName name, final IBinder binder) {
                        mediaService = ((MediaService.LocalBinder) binder).getService();
                        mediaService.setBundle(bundle);
                        mediaService.setOnClickFloatCloseBtnListener(onClickFloatCloseBtnListener);
                        mediaService.setCurrentIntent(activity.getIntent());
                        mediaService.setVodController(mVodController);
                        final SuperPlayerModelWrapper mediaPlayerModel = mediaService.getPlayerModel();
                        final String fileId = superPlayerModel.videoId.fileId;
                        if (mediaPlayerModel == null) {
                            playWithModel(superPlayerModel, hasBack, isTransparencyUiOptions);
                        } else if (fileId.equals(mediaPlayerModel.requestModel.videoId.fileId)) {
                            resumePlay(hasBack, isTransparencyUiOptions);
                        } else {
                            mediaService.stopPlay();
                            playWithModel(superPlayerModel, hasBack, isTransparencyUiOptions);
                        }
                    ...
                    public void onServiceDisconnected(final ComponentName name) {
                        mediaService.setVodController(null);
                    }
                };
                MediaService.bindMediaService(activity, serviceConnection, serviceConnection -> {
    ...
    private void resumePlay(boolean hasBack, boolean isTransparencyUiOptions) {
        this.isTransparencyUiOptions = isTransparencyUiOptions;
        if (!hasBack) {
            mVodControllerLarge.mIvBack.setVisibility(GONE);
        }
        mVodControllerSmall.mIvPause.setVisibility(VISIBLE);
        mVodControllerSmall.mIvFullScreen.setVisibility(VISIBLE);
        mVodControllerSmall.mTvDuration.setVisibility(VISIBLE);
        mVodControllerSmall.mTvCurrent.setVisibility(VISIBLE);
        mVodControllerSmall.mSeekBarProgress.setVisibility(VISIBLE);
        mVodControllerSmall.rlVideoButtomMute.setVisibility(GONE);
        editor.putInt("click", 4);
        editor.commit();
        mVodPlayer = mediaService.getTXVodPlayer();
        mVodPlayer.setVodListener(this);
        mLivePlayer = mediaService.getTXLivePlayer();
        if (mediaService.getCurrentPlayState() == SuperPlayerConst.PLAYSTATE_PLAY) {
            mPlayMode = SuperPlayerConst.PLAYMODE_FLOAT;
            requestPlayMode(SuperPlayerConst.PLAYMODE_WINDOW);
        }
    }
    ...
    private void initView(final Context context) {
        ...
        TCVodControllerFloat tcVodControllerFloat = (TCVodControllerFloat) mRootView.findViewById(R.id.controller_float);
        ...
        mRootView.removeView(tcVodControllerFloat);
        ...
        mVodControllerSmall.ivMuteTrue.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mVodPlayer.setMute(false);
                mVodControllerSmall.ivMuteFalse.setVisibility(VISIBLE);
                mVodControllerSmall.ivMuteTrue.setVisibility(GONE);
                editor.putInt("mVodPlayerMute", 1);
                editor.commit();
            }
        });


        mVodControllerSmall.ivMuteFalse.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mVodPlayer.setMute(true);
                mVodControllerSmall.ivMuteFalse.setVisibility(GONE);
                mVodControllerSmall.ivMuteTrue.setVisibility(VISIBLE);
                editor.putInt("mVodPlayerMute", 2);
                editor.commit();
            }
        });


        mVodControllerSmall.rlVideoButtomMute.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void hideTopBar() {
        mVodControllerSmall.hideTopBar();
    }
     ...
     * 显示小窗播放下顶部导航栏
     */
    public void showSmallTopBar() {
        mVodControllerSmall.showTopBar();
    }

    /**
     * 隐藏小窗播放下回退键
     */
    public void hideSmallBackButton() {
        mVodControllerSmall.hideBackButton();
    }

    /**
     * 显示小窗播放下回退键
     */
    public void showSmallBackButton() {
        mVodControllerSmall.showBackButton();
    }
    ...
    public void quitFullScreen() {
        if (isFullScreen()) {
            mVodControllerLarge.quitFullScreen();
        }
    }
    ...
    public boolean isFullScreen() {
        return mPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN;
    }
    ...
    private void initVodPlayer(Context context) {
        ...
        if (mPlayerViewCallback != null) {
            mPlayerViewCallback.onInitVodPlayer();
        }
    }
    ...
    private void initLivePlayer(Context context) {
        ...
        if (mPlayerViewCallback != null) {
            mPlayerViewCallback.onInitLivePlayer();
        }
    }


    public void playWithModel(final SuperPlayerModel modelV3, boolean hasBack, boolean isTransparencyUiOptions) {
        this.isTransparencyUiOptions = isTransparencyUiOptions;
        if (!hasBack) {
            mVodControllerLarge.mIvBack.setVisibility(GONE);
        }
        ...
        editor.putInt("click", 4);
        editor.commit();
        playWithModel(modelV3);
    }

    public void playWithModel(final SuperPlayerModel modelV3, boolean isList) {
        this.isList = isList;
        if (isList) {
            mVodControllerSmall.mIvPause.setVisibility(GONE);
            mVodControllerSmall.mIvFullScreen.setVisibility(GONE);
            mVodControllerSmall.mTvDuration.setVisibility(GONE);
            mVodControllerSmall.mTvCurrent.setVisibility(GONE);
            mVodControllerSmall.mSeekBarProgress.setVisibility(GONE);
            mVodControllerSmall.rlVideoButtomMute.setVisibility(VISIBLE);
    ...
    private void playWithModel(final SuperPlayerModel modelV3) {
        ...
        final SuperPlayerModelWrapper modelWrapper = new SuperPlayerModelWrapper(modelV3);
        playWithModel(modelWrapper);
    }

    private void playWithModel(final SuperPlayerModelWrapper modelWrapper) {
        final Context context = getContext();
        if (modelWrapper.requestModel.videoId != null) { // 根据FileId播放
            ...
            v3Loader.getVodByFileId(modelWrapper, new SuperVodInfoLoaderV3.OnVodInfoLoadListener() {
                ...
                public void onSuccess(SuperPlayerModelWrapper model) {
                    stopPlay();
                    if (mediaService != null) {
                        mediaService.setPlayerModel(model);
                    }
                    ...
                    String title = !TextUtils.isEmpty(modelWrapper.requestModel.title) ? modelWrapper.requestModel.title :
                    ...
                    mVodPlayerMute = sharedPreferences.getInt("mVodPlayerMute", 0);
                    if (isList) {
                        if (mVodPlayerMute == 1) {
                            mVodPlayer.setMute(false);
                            mVodControllerSmall.ivMuteFalse.setVisibility(VISIBLE);
            ...
            if (mCurrentModelWrapper.requestModel.multiURLs != null && !mCurrentModelWrapper.requestModel.multiURLs.isEmpty()) {// 多码率URL播放
                int i = 0;
                for (SuperPlayerModel.SuperPlayerURL superPlayerURL : mCurrentModelWrapper.requestModel.multiURLs) {
                    if (i == mCurrentModelWrapper.requestModel.playDefaultIndex) {
                ...
                mVodControllerLarge.updateVideoQulity(videoQualities.get(mCurrentModelWrapper.requestModel.playDefaultIndex));
            } else if (!TextUtils.isEmpty(mCurrentModelWrapper.requestModel.url)) { // 传统URL模式播放
                videoQualities.add(new TCVideoQulity(0, mCurrentModelWrapper.requestModel.qualityName, mCurrentModelWrapper.requestModel.url));
                videoURL = mCurrentModelWrapper.requestModel.url;
            }
            ...
            if (isRTMPPlay(videoURL)) {       // 直播播放器：普通RTMP流播放
                ...
                mReportLiveStartTime = System.currentTimeMillis();
                mLivePlayer.setPlayerView(mTXCloudVideoView);
                playTimeShiftLiveURL(mCurrentModelWrapper);
                if (mCurrentModelWrapper.requestModel.multiURLs != null && !mCurrentModelWrapper.requestModel.multiURLs.isEmpty()) {
...
//                mVodPlayer.setToken(modelWrapper.requestModel.token);
            ...
            mVodControllerSmall.updateTitle(mCurrentModelWrapper.requestModel.title);
            mVodControllerLarge.updateTitle(mCurrentModelWrapper.requestModel.title);
    ...
    private void playLiveURL(String url, int playType) {
        ...
        if (mLivePlayer != null) {
            ...
            if (result != 0) {
                ...
                if (mediaService != null) {
                    mediaService.setCurrentPlayState(SuperPlayerConst.PLAYSTATE_PLAY);
                }
     ...
     * 播放点播
     */
    private void playVodURL(String url, int intVideoTime) {
        mCurrentPlayVideoURL = url;
...
//            }
    ...
    public void stopPlay() {
        ...
        if (mediaService != null) {
            mediaService.setCurrentPlayState(SuperPlayerConst.PLAYSTATE_PAUSE);
            mediaService.stopPlay();
        }
...
//    private WindowManager.LayoutParams mWindowParams;
        ...
        if (getContext() instanceof Activity) {
            ...
            if (isFull) {
                ...
                if (Build.VERSION.SDK_INT > 11 && Build.VERSION.SDK_INT < 19) { // lower api
                    ...
                    if (isTransparencyUiOptions) {
                        decorView.setSystemUiVisibility(transparencyUiOptions);
                    } else {
    ...
    public void onBackPressed() {
        switch (mPlayMode) {
            ...
            case SuperPlayerConst.PLAYMODE_WINDOW:// 当前是窗口模式，返回退出播放器
                if (mPlayerViewCallback != null) {
                    if (mCurrentPlayState == SuperPlayerConst.PLAYSTATE_PLAY) {
                        if (mPlayerViewCallback.isShowFloat()) {
                            if (!requestOverlayPermission(
                                    activity,
                                    "小窗播放视频需要在应用设置中开启悬浮窗权限，是否前往开启权限",
                                    new OnRequestPermissionsListener() {
                                        @Override
                                        public void onPermissionsGranted() {
                                            requestPlayMode(SuperPlayerConst.PLAYMODE_FLOAT);
                                            if (mPlayerViewCallback != null) {
                                                mPlayerViewCallback.onPermissionsGranted();
                                        ...
                                        public void onPermissionsDenied() {
                                            if (mPlayerViewCallback != null) {
                                                mPlayerViewCallback.onPermissionsDenied();
                                                mPlayerViewCallback.onClickSmallReturnBtn();
                                            }
                                            if (mediaService != null) {
                                                resetPlayer();
            ...
            case SuperPlayerConst.PLAYMODE_FLOAT:// 当前是悬浮窗，退出
                if (mediaService != null) {
                    if (mediaService.onClickFloatCloseBtnListener != null) {
                        mediaService.onClickFloatCloseBtnListener.onClickFloatCloseBtn((int) mVodController.getProgress(), bundle);
                    }
                    mediaService.hideFloatPlayer();
                    resetPlayer();
                }
                break;
            default:
    ...
    public TCVodControllerBase.VodController mVodController = new TCVodControllerBase.VodController() {
        ...
        public void onRequestPlayMode(int requestPlayMode) {
            if (mVodPlayer == null && mLivePlayer == null)
                return;
            ...
            //请求悬浮窗模式
                ...
                if (mPlayerViewCallback != null && mPlayerViewCallback.isShowFloat()) {

                    requestOverlayPermission(activity, "小窗播放视频需要在应用设置中开启悬浮窗权限，是否前往开启权限", new OnRequestPermissionsListener() {
                        @Override
                        public void onPermissionsGranted() {
                            if (mPlayerViewCallback != null) {
                                mPlayerViewCallback.onPermissionsGranted();
                            ...
                            if (mediaService != null) {
                                ...
                                mediaService.showFloatPlayer(rect.x, rect.y);
                                TXCloudVideoView videoView = mediaService.getTxCloudVideoView();
                                if (videoView != null) {
                                    if (mCurrentPlayType == SuperPlayerConst.PLAYTYPE_VOD) {
...
//            fullScreen(requestPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN);
        }
        ...
        @Override
        public void onFloat(final int playMode) {
        ...
        public void resume() {
            ...
            setKeepScreenOn(true);
            mVodControllerSmall.updatePlayState(true, false);
        ...
        public void pause() {
            ...
            mVodControllerSmall.updatePlayState(false, false);
            mVodControllerLarge.updatePlayState(false);
        }

        @Override
        public void play() {
            if (isPlaying() && mPlayerViewCallback != null) {
                mPlayerViewCallback.onPlay();
            }
        }
                ...
                if (mVodPlayer != null) {
                    mVodPlayer.snapshot(new TXLivePlayer.ITXSnapshotListener() {
                        ...
                        public void onSnapshot(Bitmap bmp) {
                            if (mPlayerViewCallback != null) mPlayerViewCallback.onSnapshot(bmp);
                        }
                    });
                }
            } else {
        ...
        public void onHWAcceleration(boolean isAccelerate) {
            if (mCurrentPlayType == SuperPlayerConst.PLAYTYPE_VOD) {
                ...
                if (mVodPlayer != null) {
                    ...
                    stopPlay();
                    playVodURL(mCurrentPlayVideoURL, 5);
                }
            } else {
                if (mLivePlayer != null) {
                    mLivePlayer.enableHardwareDecode(isAccelerate);
                    playWithModel(mCurrentModelWrapper.requestModel, false);
                }
            }
            // 硬件加速上报
            if (isAccelerate) {
        ...
        public void onReplay() {
            if (!TextUtils.isEmpty(mCurrentPlayVideoURL)) {
                if (isRTMPPlay(mCurrentPlayVideoURL)) {
                    ...
                    playVodURL(mCurrentPlayVideoURL, 6);
                }
            }
     ...
     * @param bmp
     */
    public void showSnapshotWindow(final Bitmap bmp) {
...
//        popupWindow.setFocusable(true);
    ...
    private void save2MediaStore(Bitmap image) {
        ...
        try {
            File appDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).getPath());
            if (!appDir.exists()) {
                appDir.mkdir();
            }
     ...
     * @param param  2004	PLAY_EVT_PLAY_BEGIN	视频播放开始（若有转菊花效果，此时将停止）。
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        ...
        if (event != TXLiveConstants.PLAY_EVT_PLAY_PROGRESS) {
            ...
            EventBus.getDefault().post(String.valueOf(event));
        }
        if (event == TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED) { //视频播放开始
            ...
            Log.e("TXBitrateItem1", "" + TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED);
            ...
            Log.e("TXBitrateItem2", "" + TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED);
            ...
            Log.e("TXBitrateItem3", "" + TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED);
        } else if (event == TXLiveConstants.PLAY_EVT_PLAY_PROGRESS) {
            int progress = param.getInt(TXLiveConstants.EVT_PLAY_PROGRESS_MS);
            int duration = param.getInt(TXLiveConstants.EVT_PLAY_DURATION_MS);
            if (mediaService != null) {
                mediaService.updateVideoProgress(progress, duration);
            }
                ...
                if (!isWifi(getContext())) {
                    if (isShowDialog) {
                        if (isList) {
                            ...
                            showDialog(String.valueOf(param.getInt(TXLiveConstants.EVT_PLAY_DURATION_MS) / 1000));
                        }
                    }
                }
//                    }
...
//                        mVodPlayer.setToken(mCurrentModelWrapper.requestModel.token);
                    ...
                    if (url != null) {
                        playVodURL(url, param.getInt(TXLiveConstants.EVT_PLAY_DURATION_MS) / 1000);
            ...
            Log.e("TXBitrateItem5", "" + TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED);
...
//                && event != TXLiveConstants.PLAY_ERR_VOD_UNSUPPORT_DRM
            ...
            mVodControllerLarge.updatePlayState(false);
            Toast.makeText(mContext, param.getString(TXLiveConstants.EVT_DESCRIPTION), Toast.LENGTH_SHORT).show();
            Log.e("TXBitrateItem6", "" + TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED);
        }
    }
    ...
    public void onPlayEvent(int event, Bundle param) {
        ...
        if (event == TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED) { //视频播放开始
            ...
            if (mediaService != null) {
                mediaService.updateVideoProgress(progress, progress);
            }
    ...
    public void updateReplay(boolean isShow) {
        mVodControllerSmall.updateReplay(isShow);
        mVodControllerLarge.updateReplay(isShow);
    }
    ...
    public void setProgress(long current, long duration) {
        mVodControllerSmall.updateVideoProgress(current, duration);
    }

    public void setOnClickFloatCloseBtnListener(MediaService.OnClickFloatCloseBtnListener onClickFloatCloseBtnListener) {
        this.onClickFloatCloseBtnListener = onClickFloatCloseBtnListener;
    }
    ...
    public interface OnSuperPlayerViewCallback {
         ...
         * 开始全屏播放
         */
        default void onStartFullScreenPlay() {
        }

        /**
         * 结束全屏播放
         */
        default void onStopFullScreenPlay() {
        }

        /**
         * 点击小播放模式的返回按钮
         */
        default void onClickSmallReturnBtn() {
        }
         ...
         * 初始化点播
         */
        default void onInitVodPlayer() {
        }

        /**
         * 初始化直播
         */
        default void onInitLivePlayer() {
        }
    ...
    private void showDialog(String videoTime) {
        ConsumptionFlowOBAlertDialog noticeDialogAbutton = new ConsumptionFlowOBAlertDialog(getContext(), videoTime);
...
//                    mVodController.resume();
                    mVodControllerSmall.updatePlayState(true, true);
                }
            }
        });
     ...
     * @return 请求码
     */
    public synchronized static int generatePermissionRequestCode() {
        return new AtomicInteger(500).addAndGet(1);
    }
     ...
     */
    public boolean requestOverlayPermission(@NonNull Activity activity, @Nullable String direction,
                                            OnRequestPermissionsListener onRequestPermissionsListener) {
        final int originRequestCode = generatePermissionRequestCode();
        if (checkOverlayPermission(activity)) {
            ...
            builder
                    ...
                    .setPositiveButton("开启", (dialog, which) -> {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) { // 6.0动态申请悬浮窗权限
                            if (!Settings.canDrawOverlays(activity)) {
                                ...
                                final LifeCycleFragment lifeCycleFragment = LifeCycleFragment.attach(activity);
                                lifeCycleFragment.startActivityForResult(intent, originRequestCode);
                                lifeCycleFragment.addCallBack(new LifeCycleFragment.CallBack() {
                                    @Override
                            ...
                            if (!checkOp(activity, 24)) {
                                Toast.makeText(activity, "进入设置页面失败,请手动开启悬浮窗权限", Toast.LENGTH_SHORT).show();
                                onRequestPermissionsListener.onPermissionsDenied();
                                return;
                            }
                        }
                    ...
                    .setNegativeButton("暂不开启", (dialog, which) -> onRequestPermissionsListener.onPermissionsDenied())
                    .show();
            return false;
        }
    }
